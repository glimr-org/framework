...
//// to extract typed values from TOML and interpolate
//// environment variables. Without shared helpers each module
//// would reimplement the same dict lookup, type matching, and
//// ${VAR} substitution, leading to inconsistent error messages
//// and subtle differences in fallback behavior. Centralizing
//// these here guarantees all config files are parsed the same
//// way.
////
...
/// file paths are static and safe to commit. Returning a
/// default on missing or wrong-typed keys lets config files
/// stay minimal — users only override what differs from the
/// defaults without needing to specify every field.
///
...
/// Returns Result instead of a default because env-backed
/// values are typically secrets or connection strings —
/// silently falling back to a hardcoded default would cause
/// confusing runtime errors (wrong database, no Redis) instead
/// of a clear "env var not set" message at startup.
///
...
/// interpolation would need escaping rules and edge case
/// handling that aren't worth the complexity for config values.
/// Plain strings without ${} pass through unchanged so literal
/// values work without special syntax.
///
