...
/// Separating file I/O from caching lets load() decide
/// whether to read the file or use the cache. Falling back
/// to defaults on read failure avoids crashing apps that
/// don't use sessions.
///
...
/// without breaking session loading.
///
...
/// — users only need to override what differs from the
/// defaults. This keeps the minimum config file empty.
///
...
/// tom's API returns typed Toml variants, so extracting an
/// Int requires pattern matching on the table and value. A
/// helper avoids repeating this boilerplate for each integer
/// config field.
///
...
/// Keeping it separate avoids a generic helper that would
/// need dynamic typing to return different Gleam types.
///
...
/// identical config. The 120-minute lifetime balances
/// usability against the security risk of long-lived
/// sessions.
///
...
/// cost read instead of re-parsing the TOML file. The FFI
/// lives in glimr_kernel_ffi because Gleam has no native
/// persistent_term API.
///
